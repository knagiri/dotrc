...
1. **設計は確定済み**: 渡された HOW（本文 or seed 済み spec / 実装計画）を読み、そのまま
   ...
   **参照実装を読む前に base の追随を済ませる**: 委譲プロンプトが repo 内のファイルを参照実装
   ...
   `--ff-only` ではなく素の merge なのは、fast-forward できるなら git が自動で fast-forward し、分岐していればマージコミットを作るためである。
   （ローカル commit があっても常にマージコミットになるわけではない）。
...
2. **実装**: 実装計画があれば `superpowers:executing-plans` に従い、タスク単位で進める。
   ...
   **外部ツールへ渡す文字列・オプションも同じ扱いにする**: plan が外部ツールへ渡す文字列・
   オプションの仕様に踏み込んでいたら（「この文字を置換すればよい」「この `-o` を足せば塞がる」
   等）、`evidence-over-guesswork.md` §1 の「間違えると黙って壊れるクラス」に当たるかを
   確かめ、当たるなら一次情報と実効値で裏を取ってから書く。クラスの列挙はそちらが正典。
...
3. **verification**: PR を出す前に、テスト・ビルド・lint が通ることだけを確認する
   ...
   **報告する実測値は報告時点の HEAD で測り直す**: PR 本文へ数値として載せるもの（削減した
   文字数、ベンチ結果、カバレッジ等）が対象。整形・lint 修正・レビュー修正はいずれも測定の
   後に入るので、測定と報告の間に commit が挟まるのは例外ではなく既定である。ずれた値は
   テストと違って CI も判定役も検算しないため、誰も気づかないまま PR 本文に正典として残る
   （rules の由来ポインタが PR 本文を指す運用ではなおさら）。由来: dotrc#106 / dotrc#108
...
6. **委譲元への報告**: プロンプト末尾に `## 委譲元` 節（`報告先 name: <name>`）があるときは、
   ...
   - **中断**: `pr-review-automerge` が auto-merge に至らず停止したとき（同 skill の手順 4）、
     その報告の停止理由をそのまま載せる。停止経路をここで列挙しないのは、`pr-review-automerge`
     側で停止経路が増えるたびに食い違うため。
