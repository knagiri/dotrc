...
2. **自己完結プロンプトを組む**。起動先 claude は会話履歴を持たない。追加質問なしに
   ...
   `implement-and-review` の明示起動命令を置く。
   ...
   済み・未 commit で委譲先が要るファイルは `--seed <path>` で worktree 内へ入れ、相対パスで
   参照させる（詳細は `worktree-scope.md` §5）。フォーマット:
...
4. **起動する**: `claude-worktree [--seed <path>]... <name> -b <branch> -- "<prompt>"`.
