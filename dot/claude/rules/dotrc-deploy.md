...
入っていれば追記しない）。Go が入っている環境では claude-queue の `make install` は再実行の
たびに走る。

ガードが入る前に再実行した環境には、DOTRC ブロックが重複したまま残っていることがある。ガードは既存の重複を掃除しないので（ホームのファイルを書き換える話なので
スクリプトには持たせない）、気になる場合は `~/.bashrc` の重複ブロックを手で整理する。
