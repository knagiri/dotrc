...
- `--tmux` が作る tmux session 名は `<basename(メインリポジトリ toplevel)>_<name>`（正準規約。worktree dir の basename は `<name>` だけなので repo 名を合成し直している）。区切りが tmux 安全な `_` なのは従来どおりで、`.` は `tmux -t` の `window.pane` 構文と衝突するため不可
