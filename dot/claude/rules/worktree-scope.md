...
- 委譲先で token が無いときに `~/.config/gh/personal.env` 等の秘密ファイルを直接 source する回避策は使わない。`.claude/worktree-seed` 経由の供給が正規経路であり、それが効かないなら seed 側の不具合として直す。ただし現時点では、mise が config を path 単位で trust するため seed された `mise.local.toml` が新 worktree では untrusted になる点と、shell profile に mise の activate が無く非対話 login shell に `GH_TOKEN` が乗らない点の 2 つが未対応の既知 gap として残っている（解消は別 PR で扱う）。この既知 gap で token が届かないときは、回避策を撃たずに委譲元へ報告する
