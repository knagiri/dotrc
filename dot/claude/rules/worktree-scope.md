...
- `.delegate/` は `claude-worktree` が worktree 作成時に repo の exclude（`git rev-parse --git-path info/exclude` が返す `.git/info/exclude`）へ登録するので、置いたままでも `git status --porcelain` は空のままになる
  - 登録先が common dir 側になるのは、`info/` が worktree 間で共有される領域だから。`GIT_COMMON_DIR` があれば git は per-worktree の `info/` を無視して common dir 側を使うので、`.git/worktrees/<name>/info/exclude` へ書いても読まれない
