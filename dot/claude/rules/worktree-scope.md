...
- この repo の一覧から `mise.local.toml` は外した。worktree がメイン checkout の内側へ移ったことで、mise が親方向へ辿って checkout 自身の `mise.local.toml` を拾うようになり、seed する必要が消えたため（trust も親の path prefix が覆う。実測: `<repo>` だけを `mise trust` した状態で `<repo>/.worktrees/foo` から `mise env` を引くと親の値が出る）。現状この repo の既定 seed に実エントリは無いが、機構自体は残置してある
- 委譲先で token が無いときに `~/.config/gh/personal.env` 等の秘密ファイルを直接 source する回避策は使わない。`.claude/worktree-seed` 経由の供給が正規経路であり、それが効かないなら seed 側の不具合として直す。上の trust gap は `.worktrees/` 化で解消したが、**`mise activate` が非対話 shell で発火しない gap は残る**（path レイアウトとは独立）。`rc/bashrc` に activate 自体はあるが、`~/.bashrc` は非対話 shell で早期 return するうえ、activate が仕込むのは prompt hook である。実測でも、`claude-worktree` が委譲先を起こすときの `(cd "$wt_path" && claude ...)` 相当の subshell・`bash -c`・`bash -lc` のいずれでも親 config の env は載らず、`mise exec` を明示したときだけ載った。この既知 gap で token が届かないときは、回避策を撃たずに委譲元へ報告する
