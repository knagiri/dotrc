...
#### 詰まらない第 3 の形 — repo 内サブディレクトリへの `cd`

上の 2 つはいずれも working directory の外へ出る `cd` の話で、**repo 内**のサブディレクトリへの
`cd` はどちらにも当たらない。承認プロンプトも巻き戻しも起きずに通り、session の primary
working directory がそのまま移る。エラーも警告も出ないので、以降の相対パスが別の場所を
基準にしていることに気づけない。

だから「詰まらないから安全」とは読まない。詰まる形では摩擦そのものが気づきの合図になるが、
...
まま primary working directory が移った実測から。既存の 2 つはどちらも「working directory の
外へ出る」前提だったため、この形が素通りしていた。
