...
`cd` はどちらにも当たらない。承認プロンプトも巻き戻しも起きずに通り、以降の Bash 呼び出しの
cwd がそのまま移る（session の primary working directory 自体は変わらないので、移動先の
CLAUDE.md はロードされない）。エラーも警告も出ないので、以降の相対パスが別の場所を基準に
していることに気づけない。
